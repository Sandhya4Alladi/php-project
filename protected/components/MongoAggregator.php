...
class MongoAggregator
{
  ...
  private $_should_translate = false;
  private $_language;
  ...
  public function translate(bool $flag, $language)
  {
  ...
  public function aggregate(array $pipeline = [])
  {
    ...
    if ($this->_should_translate && MLFUserDataHelper::isMultiLanguage($this->_language)) {
      $pipeline = MLFUserDataHelper::addMLFStagesForUDTranslation($pipeline);
    }
    ...
    if ($this->_should_translate && MLFUserDataHelper::isMultiLanguage($this->_language)) {
      $result = MLFUserDataHelper::getTranslatedDataForAggregation($result, $this->_collection->getCollectionName(), $this->_language);
    }
    return [
      'ok' => 1.0,
      'result' => $result,
