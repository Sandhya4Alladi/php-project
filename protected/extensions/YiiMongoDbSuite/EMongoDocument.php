...
abstract class EMongoDocument extends DBDocumentPHP7
{
	...
	/**
	 * @var boolean $_should_translate to check if the document attributes need translated values.
	 */
	protected $_should_translate = false;

	/**
	 * @var string $_language is the language in which the document is to be translated.
	 */
	protected $_language;
	...
	/**
	 * This method can be invoked before invoking any kind of find query.
	 * This method sets the flag needed for the document to get translated in the langauge passed.
	 * @param boolean $flag indicates if translation is needed or not.
	 * @param string $language is the language in which the translation is needed.
	 */
	public function translate($flag = true, $lang = null){
		$this->_language = $lang ?? Yii::app()->language;
		$this->_should_translate = $flag;
		return $this;
	}
	...
	public function find($criteria=null)
	{
		...
		try{
			if($this->beforeFind())
			{
				...
				$model = $this->populateRecord($doc);
				if ($this->_should_translate && !empty($model)){
					$arr = [$model];
					$tenant_id = $criteria->getConditions()['tenant_id'] ?? Yii::app()->user->getTenantId();
					if(!empty($tenant_id)){
						MLFUserDataHelper::translateUDDocument($arr, get_class($this), $this->_language, $tenant_id);
					}
				}
				return $model;
			}
		}finally{
			$this->_should_translate = false;
			$this->_language = null;
		}
		return null;
	}
	 ...
	 * See {@link find()} for detailed explanation about $condition and $params.
	 * @param array|EMongoCriteria $condition query criteria.
	 * @return array|EMongoCursor list of documents satisfying the specified condition. An empty array is returned if none is found.
	 * @since v1.0
	 */
		...
		try{
			if($this->beforeFind())
			{
				...
				if($this->getUseCursor($criteria))
					$models =  new EMongoCursor($cursor, $this->model());
				else
					$models =  $this->populateRecords($cursor);
				if ($this->_should_translate){
					$tenant_id = $criteria->getConditions()['tenant_id'] ?? Yii::app()->user->getTenantId();
					if(!empty($tenant_id)){
						MLFUserDataHelper::translateUDDocument($models, get_class($this), $this->_language, $tenant_id);
					}
				}
				return $models;
			}
	 ...
	 * @param mixed $pk primary key value(s). Use array for multiple primary keys. For composite key, each key value must be an array (column name=>column value).
	 * @param array|EMongoCriteria $condition query criteria.
	 * @return array the document found. An null is returned if none is found.
	 * @since v1.0
	 */
	...
	{
		if ($this->_should_translate){
			$trace_back = debug_backtrace(2); // gives backtrace
			$trace_back = CJSON::encode($trace_back);
			Yii::log("Save being called on translate chaining function".$trace_back, "error"); // this will add log
			return false;
		}
	 ...
	 * @return EMongoDocument|null the newly created document. The class of the object is the same as the model class.
	 ...
	 *
	 * Returns EMongoDocumentDataProvider object ($this) with criteria set to
	...
	/**
	 * Funciton to perform find and update option together.
	 * Note:- This funciton doesn't call beforeSave and afterSave. So while using this function ensure to call beforeSave and afterSave functions accordingly
	 *
	 * @param EMongoCriteria $criteria condition to limit updating rules
	 * @param EMongoModifier $modifier updating rules to apply
	 * @param bool $new if the value is true, document after performing the modification will be returned, else document before modification will be returned
	 * @param bool $upsert if the value is true and no documents match the condition, a new document will be inserted into the collection
	 * @return EMongoDocument|null the record found or created. Null if no record is found.
	 */
	public function findAndUpdate(EMongoCriteria $criteria = null, EMongoModifier $modifier = null, bool $new = false, bool $upsert = false) {
		Yii::trace(get_class($this).'.findAndUpdate()','ext.MongoDb.EMongoDocument');
		try{
			if($this->beforeFind() && $modifier->canApply === true) {
				$this->applyScopes($criteria);
				$doc = $this->getCollection()->findOneAndUpdate(
					MongoCollectionHelper::fromLegacy($criteria->getConditions()),
					$modifier->getModifiers(),
					[
						'fields' => $criteria->getSelect(true),
						'fsync' => $this->getFsyncFlag(), 
						'w' => $this->getSafeFlag(), 
						'new' => $new,
						'upsert' => $upsert,
				...
				if ($this->_should_translate && !empty($model)){
					$arr = [$model];
					$tenant_id = $criteria->getConditions()['tenant_id'] ?? null;
					if(!empty($tenant_id)){
						MLFUserDataHelper::translateUDDocument($arr, get_class($this), $this->_language, $tenant_id);
					}
				}
